...
export class SchedulerEventsComponent extends PageComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    if (this.showData && this.widgetMode) {
      ...
      console.log('routerQueryParams', routerQueryParams);
    ...
    if (this.displayPagination) {
      this.widgetResize$ = new ResizeObserver(() => {
        const showHidePageSize = this.schedulerEventWidgetContainerRef.nativeElement.offsetWidth < 550;
  ...
  eventSourceFunction(arg: {
    ...
    if (this.schedulerEvents && this.schedulerEvents.length) {
      ...
      const userZone = null; //getUserZone();
    ...
    openSchedulerEventContextMenu($event: MouseEvent, schedulerEvent: SchedulerEventWithCustomerInfo) {
      ...
      $event.stopPropagation();
      const $element = jQuery(this.calendarContainer.nativeElement);
    ...
    private toCalendarEvent(event: SchedulerEventWithCustomerInfo, start: moment.Moment, end?: moment.Moment): EventInput {
