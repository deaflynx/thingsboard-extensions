...
export function createLabelFromDatasource(datasource, pattern: string): string {
  let label = pattern;
  if (!datasource) {
    return label;
  }
...
export function validateEntityId(entityId): boolean {
  return isDefinedAndNotNull(entityId.id) && entityId.id !== '13814000-1dd2-11b2-8080-808080808080' && isDefinedAndNotNull(entityId.entityType);
}
